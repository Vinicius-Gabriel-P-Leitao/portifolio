<script lang="ts">
	...
	const current = $derived({ color, amplitude, distance, enableMouseInteraction });
...
`;
	...
	onMount(() => {
		if (
			...
			// Skip WebGL rendering in automated test/headless environments
			return;
		}
		...
		const program = new Program(gl, {
			...
			uniforms: {
				...
				uDistance: { value: current.distance },
		...
		function update(t: number) {
			...
			program.uniforms.uDistance.value = c.distance;
