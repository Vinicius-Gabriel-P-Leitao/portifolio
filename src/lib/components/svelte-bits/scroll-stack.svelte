		...
		const setup = () => {
			if (useWindowScroll) {
				...
				lenis = new Lenis({
					...
					duration: 1.2, easing, smoothWheel: true, touchMultiplier: 2, gestureOrientation: 'vertical',
...
<div
	...
	class={`relative w-full h-full overflow-y-auto overflow-x-visible ${className}`.trim()}
...
>
	<div class="scroll-stack-inner pt-4 px-2 sm:px-6 pb-[25rem] min-h-screen">
		{#if items && children}
			{#each items as item, i (i)}
				<div
					class={`scroll-stack-card relative w-full my-6 rounded-[24px] box-border origin-top will-change-transform ${itemClass}`.trim()}
