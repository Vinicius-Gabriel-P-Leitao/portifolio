...
const Root: FC<CardRootProps> = ({
  ...
  return (
    <div
      className={clsx(
        rootVariants({ size, width, height }),
        className,
...
const Header: FC<CardHeaderProps> = ({ children, className, ...props }) => (
  <div
    className={clsx("flex flex-col items-center mb-4", className)}
    {...props}
  >
    {children}
  </div>
...
const Image: FC<CardImageProps> = ({ src, children, className, ...props }) => (
  <div
    className={clsx(
      "h-48 w-full rounded-xl mb-4 flex items-center justify-center overflow-hidden bg-indigo-900/20 p-2",
      className,
    )}
    {...props}
  >
